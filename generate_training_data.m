...
%% rull out conflicting positive data
...
for i=1:length(idx)
    for j=1:M
        if size(negative_data_cell{j},1)<2*ideal_size(j)
  ...
  phi_negative{i} = [];
      for j=1:size(positive_data_cell{i},1)
          ...
          entry=zeros(1,F);
            ...
            phi_positive{i}=[phi_positive{i};entry];
      end
      for j=1:size(negative_data_cell{i},1)
          xx=[1 negative_data_cell{i}(j,30+[2,3,4,6,7,9,10,11,12,14,15])]./[1 x_norm];
          entry=zeros(1,F);
            for k=1:size(h,1)
                entry(k)=prod(xx.^h(k,:));
            end
            entry(k+1:k+11)=negative_data_cell{i}(j,[2,3,4,6,7,9,10,11,12,14,15])./x_norm;
            phi_negative{i}=[phi_negative{i};entry];
      end
  end
