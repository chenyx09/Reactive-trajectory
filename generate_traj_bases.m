...
%         end
        for i=1:length(veh_traj_set)
            t_sample = veh_traj_set(i).t(1)+(0:Ts1:TT)*1000;
...
%             training_set=[training_set;dp];
            ...
            traj_pool = [traj_pool;delta_y x_traj];
            if any(isnan([delta_y x_traj]))
                disp('')
            end
        end
%         dp.x_traj = interp
...
traj_base=[traj_base;traj_base(:,1:m) -traj_base(:,m+1:2*m)];
N_cover = [N_cover;N_cover];
...
while i<=size(traj_base,1)-1
    ...
    while j<=size(traj_base,1)
        if scaled_inf_norm(traj_base(i,:),traj_base(j,:))<0.5*delta
